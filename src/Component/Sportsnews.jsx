...
export default function SportsPage() {
  ...
  useEffect(() => {
    ...
    syncArticlesFromServer().then((fresh) => {
      if (Array.isArray(fresh)) setArticles(getArticlesByCategory("Sports"));
    });
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10 sm:py-12">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            ...
            <div className="text-xs text-slate-500 font-medium">
              {language === "hi" ? `कुल ${toHindiNumber(articles.length)} खेल समाचार उपलब्ध` : `Total ${articles.length} Sports Stories Available`}
            </div>
          </div>
        </div>
      </section>
                ...
                <div>
                  {item.image ? (
                    <Link to={`/news/${item.id}`} className="block overflow-hidden relative">
                      <img
                        src={resolveArticleImage(item.image, item.category || "खेल", item.id || item.title)}
                        alt={item.title}
                        onError={(e) => {
                          ...
                          if (item.image && item.image.startsWith("/uploads/") && !e.currentTarget.src.startsWith("https://swadeshvaani.com")) {
                            ...
                            e.currentTarget.src = getCategoryFallbackImage(item.category || "खेल", item.id || item.title);
                          }
                        }}
