...
export default function SportsPage() {
  ...
  useEffect(() => {
    const data = getArticlesByCategory("Sports");
    setArticles(data);
  }, []);
    ...
    <main className="bg-white min-h-screen">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-6">
            <div>
              <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-orange-100 text-orange-700 text-xs font-bold uppercase tracking-wider mb-3">
                <Trophy size={14} /> खेल जगत (Sports)
              </span>
              <h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900">
                <span className="text-orange-500">Sports</span> News &amp; Updates
              </h1>
              <p className="mt-2 text-sm sm:text-base text-slate-600 max-w-2xl">
                झारखंड, राष्ट्रीय एवं अंतर्राष्ट्रीय खेल प्रतियोगिताओं, क्रिकेट, फुटबॉल और एथलेटिक्स की ताजा खबरें।
              </p>
            </div>
            <div className="text-xs text-slate-500 font-medium">
              कुल {articles.length} खेल समाचार उपलब्ध
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        <h2 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
          ...
          प्रमुख खेल सुर्खियां (Top Stories)
        </h2>
        ...
        {featured.length > 0 ? (
              ...
              >
                <div>
                  <Link to={`/news/${item.id}`} className="block overflow-hidden relative">
                    <img
                      src={item.image || "https://images.unsplash.com/photo-1517927033932-b3d18e61fb3a?auto=format&fit=crop&w=1200&q=80"}
                      alt={item.title}
                ...
                <div className="px-6 pb-6 pt-3 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                  <span className="flex items-center gap-1 font-medium">
                    ...
                    {item.reporter || item.author || "खेल डेस्क"}
                  </span>
          ...
          <p className="text-slate-500 text-sm">खेल समाचार लोड हो रहे हैं...</p>
        )}
      </section>
      ...
      {others.length > 0 && (
        <section className="max-w-6xl mx-auto px-4 sm:px-6 pb-16">
          <h2 className="text-lg font-bold text-slate-900 mb-6 flex items-center gap-2">
            <span className="h-4 w-1.5 bg-orange-500 rounded-full"></span>
            अन्य खेल समाचार (More Sports News)
          </h2>
            ...
            {others.map((item) => (
              ...
              >
                <div>
                  <Link to={`/news/${item.id}`} className="block overflow-hidden relative">
                    <img
                      src={item.image || "https://images.unsplash.com/photo-1521412644187-6c61c58bf7b3?auto=format&fit=crop&w=800&q=80"}
                      alt={item.title}
                ...
                <div className="p-4 pt-2 border-t border-slate-100 flex items-center justify-between text-[11px] text-slate-400">
                  <span className="truncate max-w-[120px]">
                    👤 {item.reporter || item.author || "खेल संवाददाता"}
                  </span>
                  <Link
                    to={`/news/${item.id}`}
